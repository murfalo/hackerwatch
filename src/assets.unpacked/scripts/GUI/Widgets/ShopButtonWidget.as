class ShopButtonWidget : ScalableSpriteButtonWidget
{
	...
	Sprite@ m_spriteIcon;
	ScriptSprite@ m_spriteScriptIcon;
	Sprite@ m_itemDot;
	Sprite@ m_itemPlus;
	...
	void SetIcon(ScriptSprite@ sprite)
	{
		@m_spriteScriptIcon = sprite;
	}

	void SetPriceGold(int amount)
	{
		@m_spriteCurrency = m_spriteGold;
		m_canAfford = Currency::CanAfford(amount);
		SetPrice(amount);
	}
	...
	void SetPriceOre(int amount)
	{
		...
		m_canAfford = Currency::CanAfford(0, amount);
		SetPrice(amount);
	}
	...
	void DrawIcon(SpriteBatch& sb, vec2 pos, vec4 color)
	{
		...
		if (m_spriteScriptIcon !is null)
			m_spriteScriptIcon.Draw(sb, pos + vec2(1, 1), g_menuTime, color);
		else if (m_spriteIcon !is null)
