class PlayerTripMine : PlayerOwnedActor
{
	...
	PlayerTripMine(UnitPtr unit, SValue& params)
	{
		super(unit, params);
	...
	void Initialize(Actor@ owner, float intensity, bool husk, uint weaponInfo = 0) override
	{
		...
		{
			auto triggerSkill = cast<CompositeActorTriggeredSkill>(m_skills[i]);
			if (triggerSkill !is null)
				@triggerSkill.m_owner = owner;
		}
	}
